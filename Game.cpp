...
public:
	...
	void SetStamina(int value = -1){ stamina += value; }
...
public:
	...
	// ЗАМЕНИТЬ НА ОДНУ ФУНКЦИЮ, КОТОРАЯ ПРИНИМАЕТ ГРАНИЦЫ ВЕРОЯТНОСТИ СОБЫТИЙ/ДВЕРЕЙ
	...
	void Generate_new_field(string type_of_door) // CHANGE NUMBER AND TYPE OF DOORS AND SET EVENT BY RANDOM
	{
	...
	void Event(string& event, Character& hero) // DEPENDING ON event SOMETHING HAPPENS
	{
		if (event == "a potion")
		{
			int random = rand() % 100;
